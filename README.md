...
The agent works in your clone. It cannot publish. You finish into one draft pull request. You get one branch, a history you can read, and a human still merges.
...
New clones default to this. `agit init --yes --mode protocol` keeps the older loop, where the agent also runs `agit commit` and `agit finish`.
...
`AGENTS.md` is a hint. Agit still writes one — and, in Cursor and Claude Code, intercepts the command before the shell runs it:

```text
agit blocked: git push is blocked in this repository.
A human publishes with agit finish <task-id>.
```

With `--mode protocol`, git mutations are redirected to agit instead of being left to raw `git`. Without those hooks, you are back to instructions.

## What agit does

`agit` is a small CLI. Publish always goes through `agit finish`. Local work is ordinary git (`remote`, the init default) or the agit commands (`protocol`).
...
**Layer 2: agent tool-call guards.** `init` wires Cursor's `beforeShellExecution` and Claude Code's `PreToolUse` hooks to `agit guard`. A blocked command gets a redirect, not a bare refusal. In `remote`, local git is allowed and publish is blocked (`git push`, mutating `gh`/`curl`/`wget`, `agit finish`). In `protocol`, git mutations are redirected to agit. The Cursor hook is fail-closed. Claude Code also gets declarative `permissions.deny` rules that match the mode.
...
`init` writes `.agit/profile.yml` with `enforcement: remote`, merges an agit section into `AGENTS.md`, installs the pre-push hook and the agent guards, and adds Cursor / Claude / Copilot instruction files. After `init`, layers 2 and 3 are already on. It does not overwrite the rest of your `AGENTS.md`, your hooks, or your existing hook configuration. It does not rewrite `origin`.
...
You get a draft PR. Review it, ask for changes, and the next `agit finish` updates the same PR:
...
To keep the protocol workflow, where the agent commits and finishes through agit:
...
If the agent starts in an empty chat, generate a prompt that matches the repo mode:
...
- It does not sandbox the agent or hold a separate credential. The guard blocks `git push` (including a literal URL), mutating `curl`/`wget` to `api.github.com`, and reading `agit.pushUrl` in an isolated clone. In `remote` it also blocks `agit finish` and mutating `gh`. A process that skips the guard and already has a URL can still push.
...
agit prompt <task-id>       Copy-paste prompt for an agent (follows enforcement)
agit install-hooks          Install the pre-push hook
agit install-agent-guards   Install the Cursor and Claude tool-call guards
```

`init --guard-only` is the same as `--mode remote`. Every command accepts `--json` so agents do not have to parse human output.
...
`enforcement: remote` lets the agent use local git and blocks publish (`git push`, mutating `gh`/`curl`/`wget`, `agit finish`). A human runs `agit finish` in their own terminal. Existing profiles without this key stay on `protocol`, where git mutations go through `agit start` / `commit` / `finish`. `agit init --yes --mode protocol` keeps that workflow. `--mode patch` is accepted; the guard currently treats it like `protocol`.
...
Try it on one repository where the agent can already change code. If agit blocks a Git action you actually wanted, open an issue with the exact command.
